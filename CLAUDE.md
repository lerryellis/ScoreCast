...
**Incident (2026-09-11/12): expired rows were never deleted, silently filled the Supabase storage quota, and took the whole app down.** `get_espn_cache_entry` only ever checked `expires_at` on *read* (treating an expired row as a cache miss) — nothing deleted the row itself. A date-keyed entry (e.g. one specific day's fixture list) is never touched again once that date passes, so every such row just sat there dead forever; months of that filled the project's 500MB quota. A full disk made even a single-row upsert take 90+ minutes (lock/write contention cascading from disk pressure), which cascaded into `resolve_predictions()` hanging, then every Supabase-touching endpoint intermittently failing with "Server disconnected"/`ConnectionTerminated`, surviving even a Supabase project restart and a Railway restart — traced over several turns before the actual `pg_stat_activity`/storage-quota check surfaced the real cause. Fixed with `database.cleanup_expired_espn_cache()` (deletes `espn_cache WHERE expires_at < now()`), run automatically once a day from the same loop that already does resolve+retrain (`api.py`'s `_auto_resolve_loop`), plus `POST /api/admin/cleanup-cache` for an on-demand run. **Separately, also fixed while chasing this**: `resolve_predictions()` had no cap on backlog size and ran ESPN fetches + Elo updates with unbounded/fully-sequential concurrency — fine at normal volume, but a multi-day backlog (from resolution silently failing during the storage-quota incident) turned one call into a multi-minute request that starved every other endpoint on its own. Now capped at `RESOLVE_BATCH_LIMIT=40` per call (oldest `match_date` first) with bounded concurrency (`ESPN_FETCH_CONCURRENCY`/`ELO_UPDATE_CONCURRENCY=8`), and the module's cached Supabase client auto-recreates with retry+backoff (`_with_stale_client_retry`, up to 4 attempts) if a connection looks dead — both worth keeping even now that the root cause is fixed, since either could recur independently.
