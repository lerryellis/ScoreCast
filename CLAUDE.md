...
**Known issue: ESPN sometimes 403s requests specifically from Railway's egress IP** (investigated live 2026-08-27). The exact same request succeeds from a normal dev/residential IP and fails only from Railway — an IP-reputation block on Railway's shared hosting range, not something triggered by this app's own request volume/pattern (confirmed by replaying the literal failing URL locally with success). **Do not try fixing this by spoofing a browser User-Agent** — already tried and verified live to make things *worse*: the same request that succeeds with httpx's default UA started failing with a Chrome UA attached, even from the previously-working dev machine. ESPN's WAF almost certainly fingerprints the mismatch between a claimed browser UA and httpx's actual TLS/HTTP handshake as more suspicious than an honest non-browser client. What's actually in place: every ESPN-calling function in the hot path (`get_espn_soccer_fixtures`, `get_espn_fixture_dates_for_month`, `get_espn_standings`) catches `httpx.HTTPError` and returns `[]`/`{}` instead of raising, so one blocked call degrades gracefully instead of 500ing the whole predictions/calendar request — this doesn't fix the block, just stops it from cascading. A real fix would need an infrastructure-level mitigation (Railway static outbound IP add-on, or routing ESPN calls through a proxy with a clean IP) — not something fixable from this codebase alone.
